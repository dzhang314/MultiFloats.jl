...
**MultiFloats.jl** provides pure-Julia implementations of arithmetic (`+`, `-`, `*`, `/`, `sqrt`), comparison (`==`, `!=`, `<`, `>`, `<=`, `>=`), floating-point introspection (`isfinite`, `eps`, `nextfloat`, etc.), and round-trip-safe float-to-string conversion. Some transcendental functions (`sqrt`, `cbrt`, `exp`, `log`, `^`) are implemented natively while others (`expm1`, `log1p`, `sin`, `cos`, etc.) are supported through [MPFR](https://www.mpfr.org/) If you have a specific trancendental function that would be useful to have a native implementation for (e.g. for GPUs) please file an issue, and we can discuss writing a native version.
